...
named!(
    time<NaiveTime>,
    do_parse!(h: number >> tag!(":") >> m: number >> (NaiveTime::from_hms(h as u32, m as u32, 0)))
);
...
named!(
    ...
    do_parse!(
        tag!("#")
            >> text: map_res!(not_line_ending, std::str::from_utf8)
            >> (text.trim().to_string())
    )
);
    ...
    map_res!(
        map_res!(
            recognize!(alt_complete!(
                delimited!(opt!(digit), tag!("."), digit) | preceded!(digit, tag!(".")) | digit
            )),
            std::str::from_utf8
        ),
        std::str::FromStr::from_str
...
named!(
    factor<f32>,
    do_parse!(tag!("-") >> f: unsigned_float >> (f))
);
...
named!(
    parts<Vec<Part>>,
    map!(separated_list!(multispace, part), |vec: Vec<_>| vec
        .into_iter()
        .collect())
);
...
mod test {
    ...
    fn test_parse_date() {
        assert_eq!(
            super::date("2017-03-21".as_bytes()),
    ...
    fn test_parse_factor() {
        assert_eq!(
            super::factor("-1.0".as_bytes()),
            IResult::Done(&b""[..], 1.0)
        );
        assert_eq!(super::factor("-1".as_bytes()), IResult::Done(&b""[..], 1.0));
    }
    ...
    fn test_parse_day() {
        ...
        let r = super::day("2017-03-20    08:00-11:30-2 12:30-17:59  #foo bar".as_bytes());
        println!("{:?}", r);
    ...
    fn test_parse_line() {
        ...
        let r = super::day("2017-03-20    08:00-11:30  12:30-17:59".as_bytes());
        println!("{:?}", r);
        assert_eq!(r, IResult::Done(&b""[..], exp_day));
    }
}
